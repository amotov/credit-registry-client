...
import com.bsbnb.creditregistry.client.core.listeners.DocumentTabDisposeListener;
import java.io.File;
...
public class DocumentAndTabManager {
    ...
    public DocumentAndTabManager(Composite composite) {
    ...
    public CTabItem saveAs(final CTabItem tab, Document document, String newPath) {
        final Document newDocument = new Document(new File(newPath),
                document.getContent(), false, document.getEncoding(), document.getMimeType());
        ...
        try {
            ...
            FileUtils.writeStringToFile(newDocument.getPath(),
                    newDocument.getContent(), newDocument.getEncoding());
            if (LOGGER.isDebugEnabled()) {
    ...
    public CTabItem save(CTabItem tab, Document document) {
        ...
        return saveAs(tab, document,
                document.getPath().getAbsolutePath());
    }
